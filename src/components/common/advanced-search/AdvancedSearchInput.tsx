...
const AdvancedSearchInput = ({ updateField, index, exp, database_name, submitSearch }: AdvancedSearchInputProps) => {
	...
	return (
		<div className="w-full flex relative m-2" key={index}>
			<Select
				...
				<SelectContent position={'popper'}>
					{searchDatabase()?.items.map((item, key) => {
						return (
							<SelectItem key={key} value={item.name} className="text-white w-full border bg-primary border-primary">
								{item.label}
							</SelectItem>
						)
					})}
			...
			/>
			{!exp.keyword && (
				<div className={'absolute w-full h-full flex items-center justify-center text-gray-500 pointer-events-none'}>
			...
			<Select
				...
				<SelectTrigger disabled={!exp.boolean} className="w-28 border border-primary bg-primary text-white rounded-l-lg font-semibold ">
					<SelectValue />
				</SelectTrigger>
				<SelectContent position={'popper'}>
					{ADVANCED_SEARCH_BOOLEAN_SELECT_MAP.map((item, key) => {
						return (
							<SelectItem key={key} value={item.value} className="text-white w-full border border-primary bg-primary">
								{item.key}
							</SelectItem>
						)
					})}
