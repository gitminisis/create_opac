...
const EventAllButton = ({ filteredEvents, contactInfo }: { filteredEvents: Cal_event[]; contactInfo: ContactInfoRSVP[] }) => {
	...
	return (
		<Dialog>
			...
			<DialogContent hideClose={'invisible'} className={'max-h-[90vh] max-w-5xl overflow-y-auto p-1 gap-1 custom-scrollbar rounded'}>
				...
				{filteredEvents?.map((elm: any, key: number) => <EventCustomDialogContent elm={elm} contactInfo={contactInfo} key={key} />)}
				<DialogFooter className={'w-full flex absolute bottom-1 relative'}>
					<DialogPrimitive.Close
						className={'mt-2 font-bold bg-primary text-primary-foreground h-10 w-20 flex items-center justify-around rounded'}>
						{message.close}
					</DialogPrimitive.Close>
				</DialogFooter>
			</DialogContent>
